...
impl FromStr for Passport {
    ...
    /// A passport is created from the string if it has all the elements of the structure.
    /// Its okay to not have the cid in some cases but all others are mandatory.
    fn from_str(s: &str) -> Result<Self, String> {
        ...
        for pair in &v {
            ...
            let x: Vec<_> = (*pair).split(":").collect();
            let k = x[0];
            let v = x[1].to_owned();
        ...
        if fields < 7 {
            return Err(format!("Less than required fileds present."));
        } else if fields == 7 && passport.cid == "" {
            Ok(passport)
        } else if fields == 8 {
            Ok(passport)
        } else {
            Err(From::from(format!("Invalid passport {:?}", passport)))
        }
    }
}
...
fn part1(input: &str) -> (i32, i32) {
    ...
    let mut extra_valid = 0;
    for line in lines {
        if line == "" {
            if let Ok(passport) = candidates.parse::<Passport>() {
                valid_passports += 1;
                if part2(&passport) {
                    extra_valid += 1;
                }
    ...
    (valid_passports, extra_valid)
}

fn is_valid_height(ht: &str) -> bool {
    if ht.ends_with("cm") {
        (150..=193).contains(&ht.replace("cm", "").parse::<usize>().unwrap())
    } else if ht.ends_with("in") {
        (59..=76).contains(&ht.replace("in", "").parse::<usize>().unwrap())
    } else {
        false
    }
}
...
fn is_valid_hair_color(hcl: &str) -> bool {
    ...
    hcl.starts_with("#")
        && hcl.chars().skip(1).all(|ch| {
            if ch.is_ascii_alphabetic() {
                chars.contains(&ch)
            } else {
                nums.contains(&ch.to_digit(10).unwrap())
            }
        })
}

/// Part2 is supposed to do some extra checks.
///
...
fn part2(passport: &Passport) -> bool {
    (1920..=2002).contains(&passport.byr.parse::<i32>().unwrap())
        && (2010..=2020).contains(&passport.iyr.parse::<i32>().unwrap())
        && (2020..=2030).contains(&passport.eyr.parse::<i32>().unwrap())
        && vec!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(&passport.ecl.as_str())
        && is_valid_hair_color(&passport.hcl)
        && is_valid_height(&passport.hgt)
        && passport.pid.chars().count() == 9
}
...
mod tests {
    #[test]
    fn t() {
        assert_eq!(super::part1(TEST).0, 2);
    }

    #[test]
    fn part1() {
        assert_eq!(super::part1(TEST2).0, 226);
    }

    #[test]
    fn part2() {
        assert_eq!(super::part1(TEST2).1, 160);
    }

    #[test]
    fn invalid_passports() {
        let s: &'static str = "eyr:1972 cid:100
...
pid:3556412378 byr:2007";
    ...
    assert_eq!(super::part1(s).1, 0);
    }

    #[test]
    fn valid_passports() {
        let s: &'static str = "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719";
    assert_eq!(super::part1(s).1, 4);
    }

    #[test]
    fn valid_hcl() {
        assert_eq!(super::is_valid_hair_color("#123abc"), true);
        assert_eq!(super::is_valid_hair_color("#123abz"), false);
        assert_eq!(super::is_valid_hair_color("123abc"), false);
    }

    #[test]
    fn valid_height() {
        assert_eq!(super::is_valid_height("60in"), true);
        assert_eq!(super::is_valid_height("190cm"), true);
        assert_eq!(super::is_valid_height("190in"), false);
        assert_eq!(super::is_valid_height("190"), false);
    }
