...
fn all_yesses(s: &Vec<char>, members: usize) -> usize {
    let mut char_ct = HashMap::new();
    
    for c in s {
        *char_ct.entry(c).or_insert(0) += 1;
    }
...
fn get_groups(s: &str) -> usize {
    let grps: Vec<_> = s.split("\n").collect();

    let mut group_uniq_sum = 0;
    let mut all_yes = 0;
    ...
    for x in grps {
        if x.is_empty() {
            // This means this is the end of the group and therfore we should calculate.
            char_set.sort();
            all_yes += all_yesses(&char_set, members);
            char_set.dedup();

            group_uniq_sum += char_set.len();
            
            char_set.clear();
            members = 0;
        } else {
            char_set.extend(x.chars());
            members += 1;
        }
    }
    char_set.sort();
...
b";
        assert_eq!(super::get_groups(s), 11);
    }

    #[test]
    fn part1() {
        assert_eq!(super::get_groups(TEST), 6809);
    }

    const TEST: &str = "lfnghcsvpyrdjtxozimb
mdtbnorpfalcijxvhsy
