...
/// }
fn parse_string_to_map(s: &str) -> HashMap<String, Vec<(i32, String)>> {
    let mut h = HashMap::new();
    let re = Regex::new(r"bags*\.*").unwrap();
    let re_count = Regex::new(r"(\d+) (.*)").unwrap();
    for line in s.split("\n") {
        let line_split: Vec<_> = line.split(" bags contain ").collect();
        let container = line_split[0];
        let v = h
            .entry(container.to_string())
            .or_insert(Vec::<(i32, String)>::new());
        for inner in line_split[1].split(", ") {
            let after = re.replace_all(inner, "");
            for cap in re_count.captures_iter(&after.trim()) {
                v.push((cap[1].parse::<i32>().unwrap(), cap[2].to_string()));
            }
        }
    }
    h
...
/// This is going through the map and construct a tree of all the ways to reaching the
/// key node from everywhere else. This returns a vec of sets.
/// The 0th level is the key itself.
/// The 1st level is all the keys, that lead to any on level 0
/// and so on.
fn create_all_paths<'a>(
    key: &'a str,
    h: &'a HashMap<String, Vec<(i32, String)>>,
) -> HashSet<&'a str> {
    // We go through all the keys multiple times until we have no more to go over.
    let mut ret = HashSet::new();
    // We enter this value initially.
    ret.insert(key);

    loop {
        let ret_before = ret.len();
        for (k, v) in h {
            for elem in v {
                if ret.contains(elem.1.as_str()) {
                    ret.insert(k.as_str());
                }
            }
        }
        let ret_after = ret.len();

        if ret_before == ret_after {
            // This means that there are no ways of getting to these nodes and that would mean they
            // are the outer most nodes.
            break;
        }
    }
    ret.remove(key);
...
fn part1(s: &str, key: &str) -> usize {
    let h = parse_string_to_map(s);
    //println!("{:?}\n", h);
    let tree = create_all_paths(key, &h);
    //println!("{:?}\n", tree);
    tree.len()
}

// fn print_me(msg: String, spaces: i32) {
//     let indent = (0..spaces).map(|_| " ").collect::<String>();
//     println!("{}{}", indent, msg);
//}
...
// returns:  The number of bags this level needs to contain inside it.
// So 0 for faded blue
//    0 for dotted black
//    11 for vibrant plum.
fn travel(key: &str, hm: &HashMap<String, Vec<(i32, String)>>, spaces: i32) -> i32 {
    let mut bags_i_contain = 0;
    for v in hm.get(&key.to_string()) {
        let sp = spaces + 2;
        for (wt, k) in v {
            let contains = travel(k, hm, sp);
            bags_i_contain += wt + wt * contains;
            // print_me(
            //     format!("{}: As a result {} contains.", bags_i_contain, key),
            //     spaces,
            // );
        }
    }
...
/// Part 2 is about the totl number of bags that will be required to fill up a shiny gold bag.
fn part2(s: &str, key: &str) -> i32 {
    let h = parse_string_to_map(s);

    // We are basically trying to create a sort of a tree structure where the root is the "shiny gold"
    // then the what are the different ways to fill it. The different colors are the nodes and the
    // count becomes the edge-weight. Let's say we represent them as a tuple (i32, &str).
    // The idea is to traverse this tree. One can do by keeping tab of nodes at a level, The value for
    // the key of the map (which is vector) and then do a DFS until you get an empty list, at which
    // point you start coming up. When you are up a level, you multiple the edge weight to it to get
    // the total bags of that color in the level above.
    //
    // Now this can also be something like this:
    //                        A
    //                       / \
    //                      B   C
    //                     / \ / \
    ...
    //                   /   / \
    //                  G   H   I
    // So remember that we want to find the number of bags, then both B and C must add the edge weight
    // But when we take H and I, we shouldn't be double counting them (once for path B -> E -> H ..)
    //  and another time for (C -> E -> H ..)
    travel(key, &h, 0)
}
...
faded blue bags contain no other bags.
dotted black bags contain no other bags.";
        assert_eq!(4, super::part1(s, "shiny gold"));
        assert_eq!(32, super::part2(s, "shiny gold"));
    }

    #[test]
    fn p1() {
        assert_eq!(278, super::part1(TEST_STR, "shiny gold"));
        assert_eq!(278, super::part2(TEST_STR, "shiny gold"));
    }
    const TEST_STR: &str =
