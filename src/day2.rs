...
/// This is a convinience method that takes a string and a pattern. The pattern is used to split
/// the string into two parts which are returned as 2-tuple.
fn my_split<'a>(pat: &'a str, line: &'a str) -> (&'a str, &'a str) {
    let x = line.split(pat).collect::<Vec<&str>>();
    (x[0], x[1])
}

impl FromStr for RuleAndExampleSet {
    type Err = ParseCharError;

    /// A typical string/line that can be used to create a RuleAndExampleSet is something like this:
    /// "1-3 a: abcde"
    /// The characters after ':' is an example.
    /// The min is 1
    /// The max is 3
    /// The character is 'a'.
    fn from_str(line: &str) -> Result<RuleAndExampleSet, Self::Err> {
        let (head, example) = my_split(": ", line);
        let (head1, c) = my_split(" ", head);
        let (min, max) = my_split("-", head1);

        Ok(RuleAndExampleSet {
            min: min.parse::<usize>().unwrap(),
            max: max.parse::<usize>().unwrap(),
            ch: c.chars().next().unwrap(),
            example: example.to_owned(),
        })
    }
}

fn does_count_lie_between(rule: &RuleAndExampleSet) -> bool {
    (rule.min..=rule.max).contains(&rule.example.matches(rule.ch).count())
}

/// https://adventofcode.com/2020/day/2
fn part1(pwds: &[&str]) -> i32 {
    let mut valid_found = 0;

    for line in pwds {
        let parsed = line.parse::<RuleAndExampleSet>().unwrap();
        if does_count_lie_between(&parsed) {
            valid_found += 1;
        }
    }
    valid_found
...
fn does_position_contain(rule: &RuleAndExampleSet) -> bool {
    let chars: Vec<char> = rule.example.chars().collect();
    // The chars vector is 0 indexed but the problem asks us to treat it as 1 indexed.
    let min = rule.min - 1;
    let first_as_expected = if min <= chars.len() {
        chars[min] == rule.ch
    } else {
        false
    };
    ...
    let max = rule.max - 1;
    let second_as_expected = if max <= chars.len() {
        chars[max] == rule.ch
    } else {
        false
    };
    ...
    // This is done is two step, check if one of them is true by OR
    // check only one true by a XOR
    (first_as_expected || second_as_expected) && (first_as_expected ^ second_as_expected)
}

fn part2(pwds: &[&str]) -> i32 {
    let mut valid_found = 0;

    for line in pwds {
        let parsed = line.parse::<RuleAndExampleSet>().unwrap();
        if does_position_contain(&parsed) {
            valid_found += 1;
        }
    }
    valid_found
...
mod tests {
    #[test]
    fn t() {
        assert_eq!(super::part1(&TEST), 2);
    }

    #[test]
    fn part1() {
        assert_eq!(super::part1(&TEST1), 519);
    }

    #[test]
    fn t2() {
        assert_eq!(super::part2(&TEST), 1);
    }

    #[test]
    fn part2() {
        assert_eq!(super::part2(&TEST1), 708);
    }
